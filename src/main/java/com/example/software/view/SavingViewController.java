    ...
    private void showSavingPlansAIPrompt() {
        try {
            ...
            if (plans == null || plans.isEmpty()) {
                showInfo("AI助手提示", "您还没有任何储蓄计划，快来制定一个储蓄目标吧！");
                return;
            }
            ...
            for (SavingPlan plan : plans) {
                ...
                sb.append(String.format("您的【%s】计划目标存钱%s%.2f，已存%s%.2f，还剩%s%.2f待存，离目标还差%d天。\n\n",
                        name,
            ...
            showAIPromptDialog("AI助手提示", sb.toString());
        } catch (Exception e) {
            System.err.println("读取储蓄计划时出错: " + e.getMessage());
            e.printStackTrace();
        }
    }
