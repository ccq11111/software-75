...
//                updateIncomeChart("Custom");
    ...
    private String getCategoryColor(String category) {
        return switch (category) {
            case "Salary" -> "#4CAF50";     // 深绿色
            case "Bonus" -> "#8BC34A";      // 浅绿色
            case "Investment" -> "#81C784";  // 中绿色
            case "Bills" -> "#FF5252";      // 红色
            case "Food" -> "#FFA726";       // 橙色
            case "Transportation" -> "#42A5F5"; // 蓝色
            case "Shopping" -> "#EC407A";    // 粉色
            case "Entertainment" -> "#AB47BC"; // 紫色
            case "Healthcare" -> "#26A69A";   // 青色
            case "Housing" -> "#5C6BC0";     // 靛蓝色
            default -> "#9E9E9E";           // 灰色
        };
    }
    ...
    private void updatePieChart(PieChart chart, List<CategorySummary> categories) {
        ...
        // 分别存储收入和支出的数据
        List<CategorySummary> incomeCategories = new ArrayList<>();
        List<CategorySummary> expendCategories = new ArrayList<>();
        ...
        for (CategorySummary category : categories) {
            if (category.getAmount() >= 0) {
                ...
                // 将负数转为正数以正确显示
                CategorySummary positiveCat = new CategorySummary(
                    category.getCategory(),
                    Math.abs(category.getAmount()),
                    category.getPercentage()
                );
                expendCategories.add(positiveCat);
            }
        }
        
        // 添加收入数据
        for (CategorySummary category : incomeCategories) {
            ...
            // 设置对应类别的颜色
            data.getNode().setStyle("-fx-pie-color: " + getCategoryColor(category.getCategory()) + ";");
        }
        
        // 添加支出数据
        for (CategorySummary category : expendCategories) {
            PieChart.Data data = new PieChart.Data(
                category.getCategory() + " (-" + String.format("%.1f%%", category.getPercentage()) + ")",
                category.getAmount()
            );
